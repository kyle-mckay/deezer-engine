...
| `pull_metadata` | `DEEZER_PULL_METADATA` | No | `true` | If `true`, fetch full track metadata when needed. Set to `false` to skip extra lookups and run with shallow metadata collection entirely to run faster, but some advanced features such as filters of non-cached data may not work.|
...
* **Entrypoint:** The image uses [`docker-entrypoint.sh`](https://codeberg.org/kylemmkay/deezer-engine/src/branch/main/docker-entrypoint.sh) as a thin wrapper that delegates execution to `python -m deezer_engine` (run/cron/pytest/shell). If you modify this script, you **must** rebuild the image.
